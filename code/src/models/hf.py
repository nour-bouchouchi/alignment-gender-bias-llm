...
def _get_active_adapters(model):
    """Return the list of active adapters in the model, or an empty list if none

    Args:
        model (Any): The PEFT model
    Returns:
        list: List of active adapter names
    """
...
def _set_lora_scale(model, scale: float):
    ...
    Args:
        model (Any): The PEFT model
        scale (float): Scaling factor for LoRA adapters
    Returns:
        None
    Notes:
...
def get_model(model_name: str, use_model_ft_dpo: bool, use_model_ft_sft: bool, lora_scale: float | None = None):
    ...
    Args:
        ...
        use_model_ft_dpo (bool): Whether DPO fine-tuning should be used
        use_model_ft_sft (bool): Whether SFT fine-tuning should be used
        lora_scale (float | None): Optional scaling factor for LoRA adapters
    Returns:
...
def merge_peft_model(model_name: str, peft_type: str) -> str:
    ...
    Args:
        ...
        peft_type (str): Type of PEFT model ('dpo' or 'sft')
    Returns:

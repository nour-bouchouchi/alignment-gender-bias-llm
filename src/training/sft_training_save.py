...
def format_sft_genderalign():
    ...
    Returns:
        list: list of dicts with 'input' and 'output' keys
    """
    sft_data = []
...
def format_sft_winobias():
    ...
    def extract_doc_id(document_id: str):
        """Build a compact document id from a Winobias document_id string

        Args:
            document_id (str): raw Winobias document identifier
        Returns:
            str: combined split label and numeric id
        """
    ...
    def load_winobias_sentences(subset_name: str):
        """Load and index Winobias sentences for a given subset

        Args:
            subset_name (str): name of the Winobias subset to load
        Returns:
            dict: mapping from document id to joined sentence text
        """
    ...
    def make_blank_prompt(pro_sentence, anti_sentence):
        """Build a gendered-blank completion prompt from a pro/anti sentence pair

        Args:
            pro_sentence (str): pro-stereotypical sentence
            anti_sentence (str): anti-stereotypical sentence
        Returns:
            str | None: instruction prompt with gendered blanks, or None if not buildable
        """
...
def format_sft_bbq():
    ...
    def map_answer_info(example):
        """Map each BBQ answer option to its normalized gender group

        Args:
            example (dict): a single BBQ example with answer_info
        Returns:
            dict: normalized group labels for ans0, ans1 and ans2
        """
        dico_answer_group = {
    ...
    def create_full_answer(answer_group, answer, question):
        """Build a natural-language answer string from a BBQ answer and its group

        Args:
            answer_group (str): normalized gender group of the answer
            answer (str): raw answer text
            question (str): the BBQ question text
        Returns:
            str: full sentence-form answer
        """
...
def load_all_dataset(selected_datasets, seed=42, eval_ratio=0.1):
    ...
    Returns:
        tuple: train and eval datasets as datasets.Dataset objects
    """
    dataset_functions = {
...
def plot_metrics(log_file: str, output_dir: str = ".") -> None:
    """Plot training and evaluation loss curves from a trainer-state JSON log

    Args:
        log_file (str): path to the trainer-state JSON log
        output_dir (str): directory where the loss plot is saved
    Returns:
        None
    """
...
def preprocess(example, tokenizer):
    ...
    Args:
        example (dict): a dict with 'input' and 'output' keys
...
def apply_lora_to_model(model):
    ...
    Returns:
        Any: the model with LoRA adapters applied
    """
...
class DataCollatorForCausalLMWithLabels:
    """Collator that pads input_ids and labels (with -100 for ignored positions) for causal-LM SFT"""
    def __init__(self, tokenizer: Any) -> None:
        """Store the tokenizer used for padding

        Args:
            tokenizer (Any): tokenizer providing the pad token id
        Returns:
            None
        """
    ...
    def __call__(self, features: list) -> dict:
        """Pad a batch of features into input_ids, attention_mask and labels tensors

        Args:
            features (list): list of tokenized feature dicts
        Returns:
            dict: padded 'input_ids', 'attention_mask' and 'labels' tensors
        """
...
def main(args: argparse.Namespace) -> None:
    """Build the SFT dataset from the selected sources and run LoRA fine-tuning

...
def compute_bias_score_from_predictions(csv_path, include_neutral=True):
    ...
    Returns:
        pd.DataFrame: bias scores for each concept
    """
...
def get_correlation_pval(method, x, y):
    """Get the correlation coefficient and p-value for a correlation test

    Args:
        method (str): the correlation method to use (e.g., "spearman")
        x (Any): the first variable
        y (Any): the second variable
    Returns:
        tuple: the correlation coefficient and the p-value for the correlation test
    """
    if method == "spearman":
...
def correlate_dot_gen(dot_folder, df_bias_score, concept, layer, method="spearman", similarity_type="dot_products"):
    ...
    Args:
        dot_folder (str): folder containing dot product CSV files
        df_bias_score (pd.DataFrame): bias scores for each concept
        concept (str): the concept to analyze
        layer (int): the layer number
        method (str): the correlation method to use
        similarity_type (str): the type of similarity measure
    Returns:
        tuple: the correlation coefficient and the p-value for the correlation test
    """
...
def plot_correlation_by_layer(df_plot, concept, save_folder, method):
    ...
    Args:
        df_plot (pd.DataFrame): correlation results
        concept (str): the concept to analyze
...
def main(args: argparse.Namespace) -> None:
    """Compute and plot per-layer Spearman correlation between latent gender scores and expressed bias
    ...
    for similarity_type in args.similarity_types:
        for concept in args.list_concepts:
            for layer in args.list_layers:
                ...
                print( f"{args.model_name}{suffix_output}")
                print(gen_csv)
                print(get_suffix_folder(args.instruction_in_prompt, args.system_prompt_key, args.model_name, args.use_model_ft_dpo, args.use_model_ft_sft))

                corr_gen, pval_gen = np.nan, np.nan
                try:
